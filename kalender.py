...
class Calendar:
    def __init__(self, parent, values):
        ...
        awl = fungsi.getTime('hari').split('-')
        print(awl)
        self.cal = calendar.TextCalendar(calendar.SUNDAY)
        self.year = int(awl[0])
        self.month = int(awl[1])
        self.wid = []
        self.day_selected = int(awl[2])
            ...
            for d, day in enumerate(week):
                if day:
                    ...
                    if day == self.day_selected:
                        b = tk.Button(self.parent, width=1, text=day, bg='red'    , command=lambda day=day:self.selection(day, calendar.day_name[(day-1) % 7]))
                    else:
